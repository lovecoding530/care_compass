...
function ul (htmlAttribs, children, convertedCSSStyles, passProps = {}) {
    const { rawChildren, nodeIndex, key, baseFontStyle, listsPrefixesRenderers, parentTag } = passProps;
    ...
    children = children && children.map((child, index) => {
        ...
        return (
            <View key={`list-${nodeIndex}-${index}-${key}`} style={{ flexDirection: 'row', marginTop: 10}}>
                { prefix }
