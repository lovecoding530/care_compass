...
export default class SingleView extends Component {
    ...
    render() {
        ...
        let levelItemStyles = (selectedLevel >= 0) ? [
            {opacity: 0.75},
            {opacity: 0.75},
            {opacity: 0.75},
        ]:[{}, {}, {}];
        ...
        return (
            <ImageBackground source={Images.bg_navigation} style={Styles.background}>
                <ScrollView contentContainerStyle={Styles.container}>
                    <Card topbar style={Styles.titleView}>
                        ...
                        <View style={Styles.progress}>
                            <Text small bold center color={Colors.Olive}>{this.state.cardIndex+1} of {this.state.cardTotalCount}  </Text>
                    ...
                    <Card style={Styles.cardView} contentStyle={{padding: 0}}>
                        <TouchableOpacity onPress={()=>{playSounds([this.state.currentCard.question_audio_url])}} style={Styles.sound_btn}>
                        ...
                        <View style={Styles.questionView}>
                            <Text medium center style={Styles.question}>{this.state.currentCard.question}</Text>
                        </View>
                        <View style={Styles.additionalInfoWrapper}>
                            <View style={Styles.triangle}/>
                            <View style={Styles.additionalInfoView}>
                                <Text center style={Styles.additionalInfo}>{this.state.currentCard.additional_info}</Text>
                            </View>      
                        </View>
                    </Card>
                ...
                <View style={Styles.buttonBar}>
                    <Button light bold onPress={()=>navigate("CDListView", {cardGame: this.state.cardGame})}>List view</Button>
                    <View style={{flex: 1}}/>
                    <Button light bold onPress={this.onSkip.bind(this)}>Skip</Button>
                    <Button dark bold onPress={this.onFinish.bind(this)}>Finish cards</Button>
                </View>
            </ImageBackground>
        );
    }
